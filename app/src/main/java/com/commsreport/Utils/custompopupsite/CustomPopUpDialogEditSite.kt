...
class CustomPopUpDialogEditSite(context: HomeActivity?, val siteList: List<SiteListModel.RowList>?, val editSiteFragment: EditSiteFragment) : Dialog(context!!) {
    ...
    private fun setreclyerview() {
        ...
        customPopupListAdapter = CustomPopupListAdapter(homeActivity!!,siteList!!,object :OnItemClickInterface{
            override fun OnItemClick(position: Int) {
                editSiteFragment.editSiteBinding!!.tvDropdownSelectsite.setText(editSiteFragment.siteList.get(position).site_name)
                editSiteFragment.selectedSiteId=editSiteFragment.siteList.get(position).id
                dismiss()
    ...
    private fun filter(text: String) {
        ...
        editSiteFragment.siteList=filterdNames
