...
class CustomPopUpDialogSiteForEmailAdd(context: HomeActivity?, val siteList: List<SiteListModel.RowList>?, val addEmailFragment: AddEmailFragment) : Dialog(context!!) {
        ...
        customPopupListAdapter = CustomPopupListAdapter(homeActivity!!,siteList!!,object :OnItemClickInterface{
            override fun OnItemClick(position: Int) {
               // faultFragment!!.faultBinding!!.contentManageFault.tvSelectedsite.setText(faultFragment.siteList.get(position).site_name)
                addEmailFragment!!.addEmailBinding!!.emailAdd.tvsite.setText(addEmailFragment.siteList.get(position).site_name)
                addEmailFragment.selectedSite=addEmailFragment.siteList.get(position).id
              //  emailFragment.callApiforEmaillist(emailFragment.siteList.get(position).id)
                dismiss()
    ...
    private fun filter(text: String) {
        ...
        addEmailFragment.siteList=filterdNames
