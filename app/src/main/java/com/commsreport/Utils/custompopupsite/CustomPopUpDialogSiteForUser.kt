...
class CustomPopUpDialogSiteForUser(context: HomeActivity?, val siteList: List<SiteListModel.RowList>?, val editUser: EditUserFragment) : Dialog(context!!) {
    ...
    private fun setreclyerview() {
        ...
        customPopupListAdapter = CustomPopupListAdapter(homeActivity!!,siteList!!,object :OnItemClickInterface{
            override fun OnItemClick(position: Int) {
                editUser.editUserBinding!!.tvDropdownSelectsite.setText(editUser.siteList.get(position).site_name)
    ...
    private fun filter(text: String) {
        ...
        editUser.siteList=filterdNames
