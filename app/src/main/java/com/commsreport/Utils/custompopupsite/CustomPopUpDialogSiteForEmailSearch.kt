...
class CustomPopUpDialogSiteForEmailSearch(context: HomeActivity?, val siteList: List<SiteListModel.RowList>?, val emailFragment: EmailFragment) : Dialog(context!!) {
        ...
        customPopupListAdapter = CustomPopupListAdapter(homeActivity!!,siteList!!,object :OnItemClickInterface{
            override fun OnItemClick(position: Int) {
               // faultFragment!!.faultBinding!!.contentManageFault.tvSelectedsite.setText(faultFragment.siteList.get(position).site_name)
                emailFragment!!.emailBinding!!.navEmailSearch!!.tvsherchName.setText(emailFragment.siteList.get(position).site_name)
                emailFragment.selectedSite=emailFragment.siteList.get(position).id
    ...
    private fun filter(text: String) {
        ...
        emailFragment.siteList=filterdNames
